...
import {
  Car, Calendar, Clock, MapPin, Eye, Phone, MessageCircle, AlertCircle, Share2,
...
export default function App() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [settingsRes, vehiclesRes, blogRes, statusRes] = await Promise.all([
        fetch('/api/settings'),
        fetch('/api/vehicles'),
        fetch('/api/blog'),
      ...
      if (!settingsRes.ok || !vehiclesRes.ok || !blogRes.ok || !statusRes.ok) {
        throw new Error("Unable to capture portal records. Reload required.");
      }
      ...
      if (sData.admin && token) {
        setAdminUser(sData.admin);
      } else {
      ...
      console.error(err);
      setAppError(err.message || "Unable to fully sync records.");
    } finally {
      setAppLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#07080a] flex items-center justify-center p-4">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <div>
            <span className="font-display font-extrabold text-white text-lg tracking-tight block">Foreclosed Auto Deals</span>
            <span className="text-[10px] font-mono text-gold-400 uppercase tracking-widest block mt-1">
  ...
  if (appError) {
    return (
      <div className="min-h-screen bg-[#07080a] flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-[#0F1115] border border-red-500/10 rounded-2xl p-6 text-center space-y-4">
          <AlertCircle className="w-12 h-12 text-red-500 mx-auto" />
          <div>
            <h2 className="font-display font-bold text-lg text-white">Database Verification Failure</h2>
            <p className="text-gray-400 text-xs mt-1.5">{appError}</p>
          </div>
          <button
            onClick={() => { setAppLoading(true); fetchAllData(); }}
            className="w-full bg-white/5 hover:bg-white/10 px-4 py-2.5 rounded-xl text-xs font-mono text-white tracking-wider border border-white/15 cursor-pointer"
          >
            REFRESH CONNECTION
          </button>
        </div>
    ...
    <div className="min-h-screen flex flex-col justify-between font-sans leading-normal overflow-x-hidden antialiased">
      ...
      {currentRoute.path === 'home' && (
        <main className="flex-grow">
          ...
          <section id="hero-showcase" className="relative min-h-[85vh] flex items-center pt-24 pb-12 overflow-hidden bg-black">
            {/* Background Image */}
            <div className="absolute inset-0 z-0">
              <img
                ...
                className="w-full h-full object-cover opacity-35 object-center scale-102 filter blur-[1px] brightness-75 scale-x-[-1]"
                referrerPolicy="no-referrer"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#07080a] via-[#07080a]/80 to-transparent" />
              <div className="absolute inset-0 bg-gradient-to-r from-[#07080a] via-transparent to-transparent" />
            </div>

            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
              <div className="max-w-3xl space-y-6">
                <div className="inline-flex items-center gap-2 bg-gold-400/10 border border-gold-500/20 px-3.5 py-1.5 rounded-full">
                ...
                <h1 className="font-display font-extrabold text-4xl sm:text-5xl md:text-6xl text-white tracking-tight leading-[1.1]">
                  Discounted Vehicles <span className="text-gold-400">Don't Stay</span> Available For Long
                </h1>

                <p className="text-gray-400 text-sm sm:text-base md:text-lg leading-relaxed font-sans max-w-2xl">
                ...
                <div className="pt-6 grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl">
                  {[
                    ...
                    <div key={i} className="border-l border-gold-500/30 pl-4 py-1">
                      <span className="text-xs font-mono text-gold-400 font-bold block uppercase tracking-wide leading-none">{item.label}</span>
                      <span className="text-[10px] text-gray-500 font-sans block mt-1">{item.sub}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <section id="real-time-ticker" className="bg-[#0f1115] border-y border-white/5 py-4 overflow-hidden relative">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center sm:justify-between gap-3 text-center sm:text-left">
              <div className="flex items-center gap-3">
                ...
                <span className="font-mono text-xs text-gray-400 uppercase tracking-widest font-medium">
                  Ledger Alert: <span className="text-white font-bold">{vehicles.filter(v => v.status === 'ACTIVE').length} Active</span> repossessed listings staged for clearing this hour.
                </span>
              </div>
              <span className="text-[10px] font-mono text-gold-500 uppercase tracking-widest bg-gold-400/5 border border-gold-500/20 px-3 py-1 rounded-full font-bold">
          ...
          <section id="listings-section" className="py-20 bg-[#07080a]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-white/5 pb-6">
                <div>
                  ...
                  <p className="text-gray-400 text-xs sm:text-sm mt-1.5 font-sans leading-relaxed">
                    Sourced exclusively from commercial auctions, debtor reclamation claims, and private bank releases.
                  </p>
                </div>

                <div className="text-xs font-mono text-gray-500 uppercase">
                  ACTIVE LEDGER STATIONS: <span className="text-gold-400 font-bold">{vehicles.length} VEHICLES TOTAL</span>
                </div>
              </div>

              {/* Dynamic Filter Controls Panel */}
              <div id="filter-bar" className="bg-[#0f1115] border border-white/5 rounded-2xl p-5 shadow-xl grid grid-cols-1 sm:grid-cols-4 gap-4">
                
                {/* Search query box */}
                <div className="space-y-1.5 sm:col-span-1">
                  <label className="text-[10px] font-mono font-bold text-gray-500 block uppercase">Search Models</label>
                  <input
                    type="text"
                    value={searchFilter}
                    onChange={(e) => setSearchFilter(e.target.value)}
                    placeholder="e.g. Mercedes, Porsche Coupe..."
                    className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-gold-500"
                  />
                </div>

                {/* Make selector dropdown */}
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono font-bold text-gray-500 block uppercase">Manufacturer</label>
                  <select
                    value={makeFilter}
                    onChange={(e) => setMakeFilter(e.target.value)}
                    className="w-full bg-[#15181d] border border-white/10 rounded-xl py-3 px-4 text-xs text-white focus:outline-none focus:border-gold-500"
                  >
                    {selectUniqueMakes.map((m, idx) => (
                      <option key={idx} value={m}>{m === 'ALL' ? 'ALL MANUFACTURERS' : m.toUpperCase()}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono font-bold text-gray-500 flex justify-between block uppercase">
                    <span>Pricing ceiling</span>
                    <span className="text-gold-400 font-medium">${priceMaxFilter.toLocaleString()}</span>
                  </label>
                  <div className="pt-2">
                    <input
                      type="range"
                      ...
                      className="w-full accent-gold-500 bg-white/10 h-1.5 rounded-lg cursor-pointer"
                    />
                  </div>
                </div>
                ...
                {/* Staged status */}
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono font-bold text-gray-500 block uppercase">Listing status</label>
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value)}
                    className="w-full bg-[#15181d] border border-white/10 rounded-xl py-3 px-4 text-xs text-white focus:outline-none"
                  >
                    <option value="ALL">ALL PORTFOLIOS</option>
                    <option value="ACTIVE">ACTIVE STAGING</option>
                    <option value="ALMOST_SOLD">ALMOST SOLD (URGENT)</option>
                    <option value="JUST_SOLD">JUST SOLD (LOCKOUT)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  .map((vehicle) => {
                    return (
                      <div
                        ...
                        className="group bg-[#0f1115] border border-white/5 hover:border-gold-500/20 rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 hover:-translate-y-1.5 cursor-pointer relative"
                      >
                        ...
                        <div className="relative aspect-[16/10] overflow-hidden bg-gray-900">
                          ...
                          {/* Sourcing identifier */}
                          <div className="absolute bottom-3 right-3 flex items-center gap-1 bg-black/60 px-2.5 py-1 rounded text-[10px] font-mono text-gray-400">
                        ...
                        <div className="p-6 space-y-4">
                          <div className="space-y-1">
                            <div className="flex justify-between items-start gap-2">
                              <h3 className="font-display font-semibold text-lg text-white group-hover:text-gold-300 transition-colors tracking-tight line-clamp-1">
                                {vehicle.title}
                              </h3>
                            </div>
                          ...
                          <div className="flex items-center justify-between border-t border-white/5 pt-4">
                            <div>
                              <span className="text-[9px] font-mono text-gray-500 block uppercase tracking-wider">PREVAILING CLEARING COST</span>
                              <span className="text-xl font-mono font-bold text-gold-400">${vehicle.price.toLocaleString()}</span>
                            </div>
                            <div className="flex items-center gap-1 text-xs font-mono font-bold text-gray-400 uppercase tracking-widest group-hover:text-gold-400 transition-all">
                ...
                {vehicles.length === 0 && (
                  <div className="col-span-full text-center py-20 border border-white/5 rounded-3xl bg-[#0f1115]">
                    <AlertCircle className="w-12 h-12 text-gray-600 mx-auto mb-4" />
                    <p className="text-base text-gray-400 font-display">No vehicle portfolios staged currently.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <section id="trust-details" className="py-20 bg-[#0f1115] border-t border-white/5">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
              <div className="text-center max-w-2xl mx-auto space-y-3">
                <h2 className="font-display font-extrabold text-2xl sm:text-3xl text-white tracking-tight">Institutional Integrity Safeguards</h2>
              ...
              <div className="grid sm:grid-cols-3 gap-8">
                {[
                  ...
                  return (
                    <div key={i} className="bg-[#07080a] border border-white/5 p-6 rounded-2xl space-y-4 shadow-xl">
                      <div className="w-10 h-10 bg-gold-400/10 border border-gold-500/20 rounded-xl flex items-center justify-center text-gold-400">
      ...
      {currentRoute.path === 'blog' && (
        <main className="flex-grow pt-24 bg-[#07080a]">
          {!currentRoute.slug ? (
            /* BLOG LIST VIEW */
            <section id="blog-search-listings" className="py-12">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/5 pb-6">
                  <div>
                    <h1 className="font-display font-bold text-3xl sm:text-4xl text-white tracking-tight">The Scribe Car Blog</h1>
                    <p className="text-xs sm:text-sm text-gray-400 font-sans leading-relaxed mt-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <input
                      ...
                      className="bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 text-xs text-white uppercase focus:outline-none focus:border-gold-500 font-mono w-56"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  {blogPosts.length === 0 && (
                    <div className="col-span-full text-center py-20 border border-white/5 rounded-3xl bg-[#0f1115]">
                      ...
                      <p className="text-base text-gray-400 font-display">No articles published yet.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              if (!post) {
                return (
                  <div className="max-w-2xl mx-auto px-4 py-20 text-center space-y-4">
                    ...
                    <h2 className="font-display font-bold text-xl text-white">Article Not Located</h2>
                    <button onClick={() => navigateTo('#/blog')} className="text-xs font-mono text-gold-400 hover:underline">
                      Return to Articles
                    </button>
                  </div>
                );
                ...
                <section id="blog-article-detail" className="py-12">
                  <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 text-xs font-mono text-gray-400 hover:text-white group transition-colors cursor-pointer uppercase"
                    >
                    ...
                    <div className="space-y-4">
                      <div className="inline-block bg-white/5 border border-white/5 px-2.5 py-1 rounded-full text-gold-400 font-mono text-[9px] uppercase tracking-widest leading-none">
                        {post.category}
                      </div>

                      <h1 className="font-display font-extrabold text-3xl sm:text-4xl md:text-5xl text-white tracking-tight leading-snug">
                        {post.title}
                      </h1>

                      {/* Reading variables */}
                      <div className="flex items-center gap-4 text-xs font-mono text-gray-500 border-b border-white/5 pb-6">
                    ...
                    {/* Featured Image */}
                    <div className="aspect-video w-full rounded-2xl overflow-hidden bg-gray-900 border border-white/5">
                      <img
                    ...
                    <div className="grid md:grid-cols-4 gap-8">
                      ...
                      <div className="md:col-span-1 space-y-6 pt-4 md:sticky md:top-28 h-fit">
                        <div className="bg-[#0f1115] border border-white/5 rounded-2xl p-4 space-y-4 text-center sm:text-left">
                          <span className="text-[9px] font-mono font-bold text-gray-500 uppercase tracking-widest block">Share Asset Guide</span>
                          ...
                          <div className="flex justify-center sm:justify-start gap-4">
                            ...
                            <button
                              ...
                              className={`p-2.5 rounded-full border transition-all cursor-pointer ${
                                copiedLink
                                  ...
                                  : 'bg-white/5 text-gray-400 border-transparent hover:text-gold-400 hover:border-gold-500/20'
                              }`}
                              title="Copy URL"
                            >
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="p-2.5 rounded-full bg-white/5 text-gray-400 hover:text-blue-400 hover:border-blue-500/10 border border-transparent transition"
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="p-2.5 rounded-full bg-white/5 text-gray-400 hover:text-sky-400 hover:border-sky-500/10 border border-transparent transition"
                            ...
                            <a
                              ...
                              className="p-2.5 rounded-full bg-white/5 text-gray-400 hover:text-amber-400 hover:border-amber-500/10 border border-transparent transition"
                              title="Email link"
                            >
                      ...
                      <div className="md:col-span-3 prose prose-invert prose-gold max-w-none">
                    ...
                    {related.length > 0 && (
                      <div className="border-t border-white/5 pt-12 mt-16 space-y-6">
                        <h3 className="font-display font-bold text-xl text-white">Recommended Asset Scribes</h3>
      ...
      {currentRoute.path === 'about' && (
        <main className="flex-grow pt-24 bg-[#07080a]">
          {/* Hero details */}
          <section id="about-hero" className="py-16 sm:py-24 relative overflow-hidden bg-black/40 border-b border-white/5">
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 space-y-4">
              <span className="text-[10px] font-mono tracking-[0.25em] text-gold-400 uppercase font-bold bg-gold-400/5 border border-gold-500/20 px-3 py-1.5 rounded-full inline-block">
                Giving Great Vehicles a Second Chance
              </span>
              <h1 className="font-display font-extrabold text-3xl sm:text-5xl text-white tracking-tight leading-snug">
          ...
          <section id="about-stats" className="py-12 bg-[#0f1115] relative">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                {[
                  ...
                  <div key={i} className="space-y-1">
                    <span className="text-2xl sm:text-4xl font-display font-extrabold text-gold-400 block tracking-tight">{stat.value}</span>
                    <span className="text-[9px] sm:text-xs font-mono uppercase text-gray-500 tracking-wider block">{stat.label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <section id="about-why-us" className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className="space-y-6">
                <h2 className="font-display font-bold text-2xl sm:text-3xl text-white tracking-tight">Why Settle Transactions With Us</h2>
                ...
                <div className="space-y-3 pt-2">
                  {[
                    ...
                    <div key={idx} className="flex gap-3 text-xs sm:text-sm text-gray-300 font-sans">
                      <span className="text-gold-500 font-bold font-mono">✔️</span>
                      <span>{benefit}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="aspect-[16/10] bg-[#0f1115] border border-white/5 rounded-2xl overflow-hidden relative shadow-2xl">
                <img
                  src="https://images.unsplash.com/photo-1618843479313-40f8afb4b4d8?auto=format"
                  alt=""
                  className="w-full h-full object-cover scale-102 filter brightness-90"
                />
              </div>
            </div>

            {/* How our process works roadmap */}
            <div id="process-blueprint" className="space-y-10 border-t border-white/5 pt-16">
              <div className="text-center max-w-xl mx-auto space-y-2">
                <h2 className="font-display font-bold text-2xl sm:text-3xl text-white tracking-tight">Efficient Acquisition Steps</h2>
                <p className="text-gray-400 text-xs sm:text-sm font-sans">Our clearance process is designed to be lean, fast, and secure.</p>
              </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                  ...
                  <div key={ix} className="bg-[#0f1115] border border-white/5 p-6 rounded-2xl space-y-4 relative overflow-hidden group">
                    <span className="absolute top-2 right-4 font-display font-extrabold text-[#ffffff03] text-7xl group-hover:text-gold-500/[0.04] transition-colors">{p.step}</span>
                    <span className="text-gold-500 font-mono text-xs font-bold block">{p.step}.</span>
                    <h4 className="font-display font-extrabold text-base text-white tracking-tight">{p.title}</h4>
            ...
            {/* FAQ segment */}
            <div id="about-faqs" className="border-t border-white/5 pt-16 space-y-10">
              <div className="text-center max-w-xl mx-auto space-y-2">
                <h2 className="font-display font-bold text-2xl sm:text-3xl text-white tracking-tight">Frequently Answered Queries</h2>
                <p className="text-gray-400 text-xs sm:text-sm">Critical legal and logistics information for prospective purchasers.</p>
              </div>
              <FAQ />
            </div>
          </section>
        ...
        (() => {
          ...
          if (!vehicle) {
            return (
              <div className="max-w-2xl mx-auto px-4 py-20 text-center space-y-4">
                <AlertCircle className="w-12 h-12 text-gold-500 mx-auto" />
                <h2 className="font-display font-bold text-xl text-white">Asset Listing Not Located</h2>
                <button onClick={() => navigateTo('#/')} className="text-xs font-mono text-gold-400 hover:underline">
                  Return to Home
                </button>
              </div>
            );
              ...
              <section id="vehicle-particulars" className="py-12">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <button
                      onClick={() => navigateTo('#/')}
                      className="flex items-center gap-2 text-xs font-mono text-gray-400 hover:text-white group uppercase"
                    >
                      <ArrowLeft className="w-4 h-4 transition-transform group-hover:-translate-x-0.5" />
                      Back to listings
                    </button>
                  ...
                  <div className="grid lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-3 space-y-4">
                      ...
                      <div className="aspect-[16/10] bg-gray-900 rounded-2xl overflow-hidden border border-white/5 relative">
                        <img
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-2">
                          {vehicle.images.map((imgUrl, index) => (
                            <button
                              ...
                              className={`w-20 sm:w-24 aspect-[16/10] rounded-xl overflow-hidden bg-gray-900 border transition-all ${
                    ...
                    <div className="lg:col-span-2 space-y-6">
                      <div className="bg-[#0f1115] border border-white/5 rounded-2xl p-6 shadow-xl space-y-6">
                        ...
                        <div className="space-y-1 border-b border-white/5 pb-4.5">
                          ...
                          <div className="flex items-baseline gap-2">
                            <span className="text-3xl sm:text-4xl font-mono font-bold text-gold-400">
                          ...
                          <div className="pt-2 flex justify-between text-[11px] font-mono text-gray-400">
                            <span>Est. Market Retail: ${estimatedRetailValue.toLocaleString()}</span>
                            <span className="text-emerald-500 font-bold">Instant Equity: +${equityProfit.toLocaleString()}</span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs">
                          <div>
                            <span className="text-[10px] font-mono text-gray-500 block uppercase">Yard Location</span>
                            <span className="text-white font-medium block mt-1">{vehicle.location}</span>
                          </div>
                          <div>
                            <span className="text-[10px] font-mono text-gray-500 block uppercase">Certified Mileage</span>
                            <span className="text-white font-medium block mt-1">{vehicle.mileage.toLocaleString()} mi</span>
                          </div>
                        </div>
                        ...
                        {vehicle.status !== 'JUST_SOLD' ? (
                          <div className="space-y-3 pt-2">
                            ...
                            <a
                              ...
                              className="w-full flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 text-white font-mono uppercase text-xs tracking-wider py-4.5 rounded-xl border border-white/10 transition-all font-bold"
                            >
                              <Phone className="w-4 h-4 text-gold-501" />
                            ...
                            <p className="text-[10px] font-mono text-amber-500 text-center uppercase tracking-wider block mt-1 animate-pulse">
                          ...
                          <div className="bg-red-500/5 border border-red-500/10 p-5 rounded-xl text-center space-y-2">
                            <span className="text-xs font-mono font-bold text-red-400 block uppercase">LOCKOUT: This vehicle has cleared.</span>
                            ...
                            <a
                              href={`tel:${settings.phone}`}
                              className="inline-block text-xs font-mono text-gold-400 hover:underline pt-1"
                            >
                  ...
                  <div className="grid lg:grid-cols-5 gap-8 border-t border-white/5 pt-12">
                    
                    {/* Specifications and descriptive details */}
                    <div className="lg:col-span-3 space-y-8">
                      <div className="space-y-4">
                        <h3 className="font-display font-bold text-xl text-white">Acquisition & Structural Diagnostics</h3>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                          {[
                            ...
                            <div key={i} className="bg-[#0e1014] border border-white/5 p-4 rounded-xl">
                              <span className="text-[9px] font-mono text-gray-500 block uppercase tracking-wide">{spec.label}</span>
                              <span className="text-white font-display font-semibold block mt-1.5 text-sm sm:text-base">{spec.val}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <h3 className="font-display font-bold text-xl text-white">Asset Description</h3>
                        <p className="text-gray-400 text-sm sm:text-base leading-relaxed font-sans text-justify bg-[#0f1115]/30 p-5 border border-white/5 rounded-2xl whitespace-pre-line">
                      ...
                      {vehicle.videoUrl ? (
                        <div className="space-y-4">
                          <h3 className="font-display font-bold text-xl text-white flex items-center gap-2">
                            ...
                            Staged Exhaust & Walk-around
                          </h3>
                          <div className="aspect-video w-full rounded-2xl overflow-hidden bg-gray-900 border border-white/5 shadow-xl relative">
                            <iframe
                        ...
                        <div className="bg-[#0f1115]/50 border border-dashed border-white/10 rounded-2xl p-6 text-center space-y-3">
                          <Video className="w-10 h-10 text-gray-600 mx-auto" />
                          <div className="text-sm font-semibold text-gray-400">Walk-around exhaust video is pending.</div>
      ...
      {/* ======================= FOOTER VIEW ===================== */}
      {/* ========================================================= */}
      <footer id="global-portal-footer" className="bg-[#0b0c0e] border-t border-white/5 py-16 text-gray-500 text-xs sm:text-sm font-sans mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
            ...
            <div className="md:col-span-2 space-y-4">
              ...
              <p className="text-gray-400 text-xs leading-relaxed max-w-sm">
                A licensed collateral reclamation & legal vehicle liquidation enterprise. Sourcing high equity repossessions, foreclosures, and loan default assets since 2012.
              </p>
              <div className="pt-2 flex items-center gap-1 text-[10px] text-gold-500 uppercase font-mono font-bold tracking-widest">
            ...
            {/* Quick routes links */}
            <div className="space-y-4">
              <span className="font-display font-medium text-white uppercase tracking-wider text-xs block">Portal Sections</span>
              <nav id="footer-routes" className="flex flex-col gap-2.5">
                <button onClick={() => navigateTo('#/')} className="text-left text-gray-400 hover:text-gold-400 transition-colors uppercase font-mono text-[11px] tracking-wide">Staged Cars</button>
                <button onClick={() => navigateTo('#/blog')} className="text-left text-gray-400 hover:text-gold-400 transition-colors uppercase font-mono text-[11px] tracking-wide">SEO Blog Articles</button>
                <button onClick={() => navigateTo('#/about')} className="text-left text-gray-400 hover:text-gold-400 transition-colors uppercase font-mono text-[11px] tracking-wide">Trust & Process</button>
                <button onClick={() => navigateTo('#/admin')} className="text-left text-gray-400 hover:text-gold-400 transition-colors uppercase font-mono text-[11px] tracking-wide">Admin Portal</button>
              </nav>
            </div>

            {/* Coordinates */}
            <div className="space-y-4">
              <span className="font-display font-medium text-white uppercase tracking-wider text-xs block">Liquidation Hub</span>
              <div className="space-y-3 leading-relaxed text-xs text-gray-400">
                <div className="flex items-start gap-2.5">
                  <MapPin className="w-4 h-4 text-gold-500/70 flex-shrink-0" />
          ...
          <div className="border-t border-white/5 mt-12 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-[11px] text-gray-600 font-mono">
            <span>© {new Date().getFullYear()} {settings.companyName}. All rights and ownership titles reserved.</span>
            <div className="flex gap-4">
              <button onClick={() => navigateTo('#/admin')} className="hover:text-gold-400 transition">Secure Panel</button>
              <span>•</span>
